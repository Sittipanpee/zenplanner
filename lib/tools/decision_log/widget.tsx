...
import { decisionLogSchema, todayIso } from "./index";

export function DecisionLogWidget({ entries, onEntryAdd }: ToolWidgetProps) {
  const t = useTranslations("tools.decisionLog");
  ...
  const initial = useMemo(() => {
    ...
    const p = decisionLogSchema.safeParse(todayEntry.payload);
  ...
  return (
    <div className="flex w-full max-w-md flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <div className="flex items-center gap-2">
        <ScrollText className="h-4 w-4 text-zen-indigo" />
      ...
      {items.length < 10 && (
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={add} disabled={!draft.trim()} aria-label="Add"
            className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-zen-indigo text-white disabled:opacity-40">
