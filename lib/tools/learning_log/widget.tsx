...
import { learningLogSchema, todayIso } from "./index";

export function LearningLogWidget({ entries, onEntryAdd }: ToolWidgetProps) {
  const t = useTranslations("tools.learningLog");
  ...
  const initial = useMemo(() => {
    ...
    const p = learningLogSchema.safeParse(todayEntry.payload);
  ...
  return (
    <div className="flex w-full max-w-md flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <div className="flex items-center gap-2">
        <Briefcase className="h-4 w-4 text-zen-sage" />
