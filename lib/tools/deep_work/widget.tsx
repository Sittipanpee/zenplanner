...
import { deepWorkSchema, todayIso } from "./index";

export function DeepWorkWidget({ entries, onEntryAdd }: ToolWidgetProps) {
  const t = useTranslations("tools.deepWork");
  ...
  const initial = useMemo(() => {
    ...
    const p = deepWorkSchema.safeParse(todayEntry.payload);
  ...
  return (
    <div className="flex w-full max-w-md flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <div className="flex items-center gap-2">
        <Brain className="h-4 w-4 text-zen-indigo" />
