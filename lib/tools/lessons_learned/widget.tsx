...
import { lessonsLearnedSchema, todayIso } from "./index";

export function LessonsLearnedWidget({ entries, onEntryAdd }: ToolWidgetProps) {
  const t = useTranslations("tools.lessonsLearned");
  ...
  const initial = useMemo(() => {
    ...
    const p = lessonsLearnedSchema.safeParse(todayEntry.payload);
  ...
  return (
    <div className="flex w-full max-w-md flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <div className="flex items-center gap-2">
        <Lightbulb className="h-4 w-4 text-zen-amber" />
      ...
      {items.length < 10 && (
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={add} disabled={!draft.trim()} aria-label="Add"
            className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-zen-amber text-white disabled:opacity-40">
