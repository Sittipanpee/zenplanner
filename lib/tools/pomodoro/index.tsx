...
const DEFAULT_CONFIG: PomodoroConfig = { sessionLength: 25, breakLength: 5 };
...
function PomodoroWidget({ config, entries, onEntryAdd, onConfigChange }: ToolWidgetProps) {
  const t = useTranslations("tools.pomodoro");

  const cfg: PomodoroConfig = {
    sessionLength:
      typeof config.sessionLength === "number" ? config.sessionLength : DEFAULT_CONFIG.sessionLength,
    breakLength:
      typeof config.breakLength === "number" ? config.breakLength : DEFAULT_CONFIG.breakLength,
  };
  ...
  // Reset timer when config or mode changes (when not running)
  useEffect(() => {
    if (!running) {
  ...
  const handleComplete = useCallback(async () => {
    ...
    if (mode === "work") {
      try {
        ...
        // swallow — entry add failure should not crash widget
      }
      setMode("break");
    ...
    intervalRef.current = setInterval(() => {
      setSecondsLeft((s) => {
        if (s <= 1) {
          ...
          // defer state update outside reducer
          setTimeout(() => {
            void handleComplete();
          }, 0);
          return 0;
